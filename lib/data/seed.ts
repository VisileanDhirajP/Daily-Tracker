...
/** One seed entry: how many days back, plus the entry payload. */
export type SeedSpec = { dayOffset: number; input: Omit<EntryInput, "entry_date"> };

/**
 * Turn a list of specs into concrete entries for a user, dated relative to
 * today. IDs are prefixed with the user id so they stay unique even when
 * several users' entries are combined into one list (the manager team feed).
 */
export function buildEntriesFromSpecs(userId: string, specs: SeedSpec[]): Entry[] {
  const t = todayISO();
  const now = new Date().toISOString();
  return specs.map((spec, i) => ({
    id: `${userId}-seed-${i + 1}`,
    user_id: userId,
    entry_date: shiftDay(t, -spec.dayOffset),
    created_at: now,
    updated_at: now,
    ...spec.input,
  }));
...
/** Default ~2-week demo set for the primary mock employee. */
...
/**
 * Generate ~2 weeks of demo entries for the primary mock user so the dashboard
 * is populated on first run. Dates are relative to today.
 */
export function buildSeedEntries(userId: string): Entry[] {
  return buildEntriesFromSpecs(userId, DEFAULT_SPECS);
}
